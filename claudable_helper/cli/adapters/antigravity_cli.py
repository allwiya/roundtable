"""Antigravity CLI adapter for Roundtable AI MCP Server."""
...
class AntigravityCLI(BaseCLI):
    ...
    async def check_availability(self) -> Dict[str, Any]:
        try:
            ...
            return {"available": proc.returncode == 0, "status": "✅ Antigravity CLI Available" if proc.returncode == 0 else f"❌ Antigravity CLI failed"}
    ...
    async def execute_with_streaming(self, instruction: str, project_path: str, session_id: Optional[str] = None, model: Optional[str] = None, images: Optional[List[Dict[str, Any]]] = None, is_initial_prompt: bool = False) -> AsyncIterator[Message]:
        ...
        cmd = ["antigravity", instruction, "--project", project_path]
        try:
